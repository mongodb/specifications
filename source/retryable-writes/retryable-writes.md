...
Transaction ID : The transaction ID identifies the transaction as part of which the command is running. In a write
...
ClientSession : Driver object representing a client session, which is defined in the
...
Retryable Error : An error is considered retryable if it has a RetryableWriteError label in its top-level "errorLabels"
field. See [Determining Retryable Errors](#determining-retryable-errors) for more information.
