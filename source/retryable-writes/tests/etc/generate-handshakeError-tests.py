...
RETRYABLE_WRITE_OPERATIONS = [op for op in OPERATIONS if op.operation_name in
                              ['insertOne', 'updateOne', 'deleteOne', 'replaceOne', 'findOneAndDelete',
                                  'findOneAndUpdate', 'findOneAndReplace', 'insertMany', 'bulkWrite']
                              ]


# ./source/retryable-writes/tests/etc
...
def generate_handshake_error_tests():
    generate('handshakeError', RETRYABLE_WRITE_OPERATIONS)
