...
If the ``sasl`` step(s) fail and callbacks are in use, the driver
MUST clear the cache value and attempt to authenticate one more time starting
from ``OIDCMechanismClientStep1``.  In this case the identity provider may have
