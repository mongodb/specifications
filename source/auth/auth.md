...
- mechanism_properties
  ...
  - ENVIRONMENT\
    ...
    to use to obtain credentials. If provided, the value MUST be one of `["test", "azure", "gcp", "k8s"]`. If both
  ...
  - TOKEN_RESOURCE\
    ...
    `["azure", "gcp", "k8s"]` or `TOKEN_RESOURCE` is not provided and `ENVIRONMENT` is one of `["azure", "gcp", "k8s"]`,
    the driver MUST raise an error. Note: because the `TOKEN_RESOURCE` is often itself a URL, drivers MUST document that
    a `TOKEN_RESOURCE` with a comma `,` must be given as a `MongoClient` configuration and not as part of the connection
...
- 2024-07-29: Add Kubernetes built-in OIDC provider integration.
