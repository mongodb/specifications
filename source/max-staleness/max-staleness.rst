...
It MUST be configurable similar to other read preference options like "mode"
and "tag_sets". Clients MUST also recognize it in the connection string::

  mongodb://host/?maxStalenessMS=30000
...
We wish to prohibit this effect.
We also do not want users to expect greater precision than the staleness estimate offers,
so we arbitrarily chose twice heartbeatFrequencyMS as the lower bound for maxStalenessMS.
