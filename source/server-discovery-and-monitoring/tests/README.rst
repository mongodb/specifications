...
- type: A ServerType name, like "RSSecondary". For async and multi-threaded implementations, the "PossiblePrimary" type should be interpreted as "RSUnknown". See `ServerType <../server-discovery-and-monitoring.rst#servertype>`_ for more details.
- setName: A string with the expected replica set name, or null.
