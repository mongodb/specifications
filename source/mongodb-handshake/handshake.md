...
hello command : The command named `hello`. It is the preferred and modern command for handshakes and topology
monitoring.

legacy hello command : The command named `isMaster`. It is the deprecated equivalent of the `hello` command. It was
deprecated in MongoDB 5.0.

isMaster / ismaster : The correct casing is `isMaster`, but servers will accept the alternate casing `ismaster`. Other
case variations result in `CommandNotFound`. Drivers MUST take this case variation into account when determining which
